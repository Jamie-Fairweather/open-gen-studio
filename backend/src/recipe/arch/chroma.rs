...
/// Chroma (Flux-derived): UNET + CLIP(chroma/t5) + Flux VAE + EmptySD3Latent + AuraFlow.
/// Uses real CFG + negative (unlike Flux.1 guidance path).
pub(crate) fn compile_chroma(
    manifest: &ManifestFile,
    values: &HashMap<String, Value>,
) -> Result<Value, String> {
    let unet = model_by_role(&manifest.models, "unet")?;
    let te = model_by_role(&manifest.models, "text_encoder")
        .or_else(|_| model_by_role(&manifest.models, "t5"))
    ...
    let steps = i64_val(values, "steps", 26);
    let cfg = f64_val(values, "cfg", 4.0);
