...
pub fn run_image_to_prompt(
    ...
    let job = {
        let db = state.db.lock().map_err(|e| e.to_string())?;
        let created = db.create_job("prompt-tool", &params)?;
        db.update_job_status(&created.id, "running", None)?
    };
    let _ = app.emit("jobs://updated", &job);
    {
    ...
    std::thread::spawn(move || {
        let state = app_bg.state::<AppState>();
        let result = prompt_tools::run_image_to_prompt(
            &app_bg,
            &state.db,
            &state.processes,
            &state.cancelled_jobs,
...
pub fn run_prompt_enhance(
    ...
    std::thread::spawn(move || {
        let state = app_bg.state::<AppState>();
        let result = prompt_tools::run_prompt_enhance(
            &app_bg,
            &state.db,
            &state.processes,
            &state.cancelled_jobs,
