...
- **`tauri-specta`** collects commands (`commands::specta_builder`) and exports `frontend/lib/generated/bindings.ts` (typed `commands.*` + DTO types).
...
- Opaque JSON (`serde_json::Value`) must not be inlined by Specta - use `#[specta(type = specta_typescript::Any)]` on fields, or `JsonValue` / `JsonMap` at command boundaries (`backend/src/json_any.rs`).
- `RECIPE_ARCHES` is appended from `RecipeArch::ALL` after export.
- Prefer `@/lib/generated/bindings` (or `lib/host.ts` / `lib/arch.ts` re-exports). `frontend/lib/generated/ipc.ts` is a thin re-export for old imports.

---

## When stable v2 is out - checklist

1. Bump `specta`, `tauri-specta`, `specta-typescript`, `specta-serde` in `backend/Cargo.toml` to the stable line.
