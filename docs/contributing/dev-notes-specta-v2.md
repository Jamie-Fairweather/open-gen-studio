...
**Ship path:** when **2.0 stable** lands, bump pins and re-run `npm run ipc:types` — expect little churn from rc.25.
...
- **`npm run ipc:types`** → `cargo run --bin export-ipc` (not a unit test — Windows lib tests can fail with `STATUS_ENTRYPOINT_NOT_FOUND` when linking heavy Tauri/specta).
...
2. `npm run ipc:types` — skim `bindings.ts` for shape drift.
3. `npm run typecheck` + smoke a few invokes in the app.
